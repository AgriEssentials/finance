...
class SentimentAnalyzer:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self.max_articles_to_analyze = 25  # Cap per-article sentiment analysis latency
        self.min_articles_to_analyze = 50
        self.max_groq_sentiment_articles = 10  # Cap LLM sentiment calls per stock (rate limits + speed)
    ...
    def _initialize_transformer_model(self):
        ...
        # Groq rate-limit circuit breaker (avoids hammering a 429-limited API)
        self._groq_blocked_until = 0.0
    ...
    def analyze_sentiment(self, text: str) -> Dict[str, Any]:
        ...
        # Circuit breaker: skip Groq while it is rate-limited (usually 60-120s)
        import time as _time
        if _time.time() >= self._groq_blocked_until:
            try:
    ...
    def _analyze_sentiment_groq(self, text: str) -> Optional[Dict[str, Any]]:
        """Analyze sentiment using Groq's LLM API (with rate-limit circuit breaker)"""
        try:
            if not self.groq_api_key:
                return None
            # Circuit breaker active - skip without burning time
            import time as _time
            if _time.time() < self._groq_blocked_until:
                return None
...
- Consider impact on stock price (positive news may be negative for competitors)"""
            ...
            # Circuit breaker: after consecutive 429s, stop calling Groq for a while
            if response is not None and response.status_code == 429:
                self._groq_consecutive_429 += 1
                if self._groq_consecutive_429 >= 2:
                    self._groq_blocked_until = time.time() + 120
                    print("[GROQ SENTIMENT] Circuit breaker opened - skipping Groq for 120s")
    ...
    def aggregate_sentiment(self, texts: List[str]) -> Dict[str, Any]:
        """Aggregate sentiment across multiple texts (parallel per-article analysis)"""
        if not texts:
            return {
                "average_score": 0,
        ...
        # Analyze articles concurrently for speed (thread-safe: uses own HTTP/torch calls)
        from concurrent.futures import ThreadPoolExecutor
        worker_count = min(4, len(valid_texts))
        scores = []
        positive = 0
        negative = 0
        neutral = 0
        ...
        try:
            with ThreadPoolExecutor(max_workers=worker_count) as pool:
                results = list(pool.map(self._safe_sentiment, valid_texts))
            ...
            # Sequential fallback if threading fails
    ...
    def _safe_sentiment(self, text: str) -> float:
        """Thread-safe single-article sentiment score (never raises)."""
        try:
            result = self.analyze_sentiment(text)
    ...
    async def _fetch_news_headlines_firecrawl_async(self, session: aiohttp.ClientSession, company_name: str, clean_symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            async with session.post(self.firecrawl_url, headers=headers, json=payload, timeout=aiohttp.ClientTimeout(total=15)) as response:
                if response.status == 200:
                    data = await response.json()
                    articles = []
    ...
    def _fetch_news_headlines_firecrawl(self, company_name: str, clean_symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            response = requests.post(self.firecrawl_url, headers=headers, json=payload, timeout=15)
            if response.status_code == 200:
                articles = []
    ...
    def fetch_news_headlines(self, symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Fallback to sync version - fetch all providers concurrently for speed
            context = self._get_company_context(symbol)
            clean_symbol = context["clean_symbol"]
            company_name = context["company_name"]
            ...
            with ThreadPoolExecutor(max_workers=4) as pool:
                f_gnews = pool.submit(self._fetch_news_headlines_gnews, company_name, clean_symbol)
                f_finnhub = pool.submit(self._fetch_news_headlines_finnhub, symbol, company_name, clean_symbol)
                f_newsdata = pool.submit(self._fetch_news_headlines_newsdata, company_name, clean_symbol)
                f_firecrawl = pool.submit(self._fetch_news_headlines_firecrawl, company_name, clean_symbol)
                gnews_articles = f_gnews.result(timeout=12)
                finnhub_articles = f_finnhub.result(timeout=12)
                newsdata_articles = f_newsdata.result(timeout=12)
                firecrawl_articles = f_firecrawl.result(timeout=25)
