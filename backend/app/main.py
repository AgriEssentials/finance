...
async def startup_event():
    ...
    # Seed RAG document corpus + warm models (background threads - non-blocking)
    import asyncio
    asyncio.create_task(asyncio.to_thread(_seed_rag_corpus))
    asyncio.create_task(asyncio.to_thread(_warm_models))
    
    print("Application started successfully")

def _seed_rag_corpus():
...
def _warm_models():
    ...
    Runs in a worker thread so the event loop stays responsive. Warms:
      - sentiment transformer (avoids the 60s lazy-load in worker threads)
      - RAG embedding engine (distilbert / TF-IDF fallback)
    """
    try:
        from app.sentiment import sentiment_analyzer
        ...
        print(f"[STARTUP] Sentiment warm failed: {e}")
    try:
        from app.agents.rag import corpus
        corpus._embedder._load_transformer()
        print(f"[STARTUP] RAG embedding model warm ({corpus._embedder.model_name})")
    except Exception as e:
        print(f"[STARTUP] RAG warm failed: {e}")
